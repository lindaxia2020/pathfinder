...
**Internal flow**:
```
Path A (ATS API, for Greenhouse/Lever/Ashby/Workable/Workday + site adapters Amazon/Google/Microsoft):
  → Call public JSON API to get full job listings (no auth required)
  → Local filtering: job title contains TPM keywords (track-aware) + location in WA/CA/TX/US-Remote (+FL Space)
  → Amazon/Google/Ashby records carry prefetched JD text (no browser render); Workday/Microsoft JDs via page JSON-LD
  → Pre-scrape freshness gate: first-seen postings older than the track window (14 d Mid-large / 45 d vertical) are skipped
...
|-----------|-------------|--------|
| `Company_List` | Company Name | Company Name, **Track** (6-bucket taxonomy — PRJ-004), Business Focus, Career URL, Updated At, TPM Jobs, **Qualified Jobs** (PRJ-004) — `No TPM Count`/`Auto Archived` removed 2026-08-21 (REQ-167) |
...
- ~~Archive management~~ — removed 2026-08-21 (REQ-167); the Company_List migration strips the legacy columns (`_LEGACY_ARCHIVE_COLUMNS`)
...
- **Cross-process safety (BUG-73)**: `shared/run_lock.py` — every agent entry point holds an exclusive `flock` on `logs/pathfinder.lock` for its whole run; a second agent fails fast naming the holder, or queues with `PATHFINDER_LOCK_WAIT=1` (exported by the scheduled pipeline). Kernel releases the lock on any exit — no stale locks after a crash
- **Snapshot reads (BUG-73)**: all `read_only` workbook loads go through `load_workbook_readonly` — file bytes copied to memory before parsing (a concurrent in-place `wb.save()` can't crash a reader mid-stream; half-written files retried 3×)
...
| v2.6 (current) | 2026-08-22 | **Open follow-ups cleared (BUG-83~85, REQ-168~172)**. job_agent: `_fetch_google_jobs` honors Career-URL `company=` params (request filter + payload index-7 check); new Microsoft discovery adapter `_fetch_microsoft_jobs` on Eightfold's public `pcsx/search` API (`ATS_PLATFORMS["microsoft"]` → `json_api`, ATS-trusted path; 2 s pacing + 429 same-page back-off) with JSON-LD-first `_scrape_microsoft_jd`; `_detect_ats` skips adapter-only platforms; `_parse_jsonld_jobposting` accepts nested address objects; `_apply_prescrape_freshness_gate(track)` uses per-track first-seen windows from `shared/config.py` (`FIRST_SEEN_MAX_AGE_DAYS`=14 / `_VERTICAL`=45); `TPM_KW` + "technical program management"; Ashby list records carry `_prefetched_md`. No routing-flow change beyond Microsoft moving from Path B to Path A. | Result-review follow-ups: the DeepMind row absorbed Google-wide postings; Microsoft (a primary target) yielded ~1 row/run from Path B; Cowboy Space's 07-17 TPM was lost to the 14-day first-seen window; Salesforce's "Technical Program Management" titles never matched; Ashby JDs needed browser renders although the API had the body. |
| v2.5 | 2026-08-21 | **Auto-archive feature removed (REQ-167, was REQ-063)**. job_agent no longer skips `Auto Archived` companies nor runs the post-run archive phase; `AUTO_ARCHIVE_THRESHOLD` and the five archive helpers in `shared/excel_store.py` deleted; `COMPANY_HEADERS` back to 7 columns; `get_or_create_excel` migration deletes legacy `No TPM Count`/`Auto Archived` columns from existing workbooks. | User decision: a company with no TPM jobs today may post one tomorrow — never skip it. Two bug-affected runs (BUG-79~82) had also pushed 334 companies to one run short of silent archival, and triaging all of a company's rows out of JD_Tracker looked like "no jobs" to the archiver. |
| v2.4 | 2026-08-21 | **Write-time geo robustness (BUG-82) + YoE policy (REQ-166)**. `classify_region` normalizes separators and recognizes leading state codes (`WA - …`, `CA - Remote`) and token-based remote forms (`US - Remote`, `Remote (United States)`); `_gate_and_finalize` takes `list_location` (list-API structured location, threaded from `process_company` via `_process_scraped_jd`) and drops only when both extracted and list locations are out of scope. YoE gate: skip only stated min ≥11 (lower cut removed). | Gemini formats Workday's "WA - <building>, <country>" inconsistently (verbatim vs rewritten); the verbatim form was geo-dropped at write time although the ATS list had already placed the posting in Greater Seattle. A stated "2-5 yrs" TPM at an early-stage space company (Cowboy Space) is in the user's scope. |
| v2.3 | 2026-08-20 | **Job-agent recall fixes (BUG-79~81)**. `classify_region`: WA now also recognized from full-state-name forms (`"<city>, Washington[, country]"`) and unambiguous WA city hints (incl. "Greater Seattle Area"), vetoed by other-state/province names and guarded by `_DC_RE`; CA/TX/FL state tokens word-bounded (`", ca"` ≠ `", canada"`). `_fetch_workday_jobs`: `/en-US/<site>` locale segment skipped when deriving the CXS site slug; `total` captured from page 0 only (later pages report 0) so REQ-004-13 uncapped pagination actually runs. No routing/flow change. | User's review of a manual run: Amazon's 177 Washington TPM postings ("Seattle, Washington, USA"), Microsoft's "Redmond, Washington, United States" and Ashby/Workday "Greater Seattle Area" rows were all silently geo-dropped; 13 Workday rows returned 404 and every Workday company was capped at 40 postings. |
| v2.2 | 2026-08-19 | **Filter widening: titles + WA geo**. Title filter: `TPM_KW` gains "Technical Project Manager" (+ variants) on all tracks; new `PM_TITLE_OK_TRACKS` (AI-native/Robotics/Space/Defense) additionally accepts plain "Program Manager" in `_tpm_filter` (track threaded `process_company → discover_jobs → _discover_via_api`) and in the `llm_filter_jobs` rules (three-way split: PM-OK vertical / Fintech / mid-large). Server-side queries: Workday `searchText` + Firecrawl map broadened to "Program Manager"; Amazon/Google deliberately kept narrow (pagination caps). Geo: `classify_region` region "Seattle" → "WA" — any `", WA"` state token (word-boundary regex) or explicit Washington-state form; keep-set now WA/CA/TX/US-Remote, sort-tier grouping WA+Remote > CA/TX. Company discovery aligned to the same scope (GEOGRAPHY prompt clause: WA state + US-remote qualification; `TAVILY_QUERIES` = per-track bases + one uniform `_QUERY_GEO_TAIL`, no per-track geo — both soft biases, no deterministic company-level filter; +`_SPACE_REGION_QUERIES` geo-targeted Space exception). FL = Space-track-only region (REQ-162): `classify_region` gains "FL", track-aware keep via shared `_geo_out_of_scope(location, track)` in both geo gates, GEOGRAPHY clause Space-FL exception, sort tier FL grouped with CA/TX. Tests 945+ → 1,149. | Title conventions vary ("Technical Project Manager" = same role at many companies; vertical-track orgs title technical roles plain "Program Manager" — small orgs, no non-tech PM bureaucracy), and the user's real commutable scope is all of WA (Bellevue/Redmond/Everett/Kent — Boeing, Blue Origin sites), not just the Seattle-metro city list. Recall widened at the cheap pre-scrape stage; write-time gates (domain/YoE/work-auth) keep final precision. |
